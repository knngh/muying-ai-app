<template>
  <view class="shortcut-editor">
    ...
    <button class="reset-button" @tap="emit('reset')">恢复阶段推荐</button>
  </view>
</template>
<script setup lang="ts">
import { getToolDefinition, type ToolId } from '@/data/tool-catalog'
import { MAX_HOME_TOOLS, moveHomeTool } from '@/utils/home-tools'
defineProps<{ ids: ToolId[] }>()
const emit = defineEmits<{ change: [ids: ToolId[]]; reset: [] }>()
</script>
<style scoped>
