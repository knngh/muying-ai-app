<template>
  <view v-if="TOOL_AI_ENABLED" class="tool-panel expense-candidates">
    ...
    <form @submit="submit">
      <textarea v-model="text" name="text" class="candidate-input" maxlength="500" :disabled="busy" placeholder="例如：奶粉 268，尿布 89" />
    ...
    <form v-for="candidate in candidates" :key="candidate.id" class="candidate-row" :class="{ 'candidate-row--saved': savedIds.includes(candidate.id) }" @submit="saveCandidate(candidate, $event)">
      ...
      <template v-else>
        ...
        <text v-if="duplicateIds.includes(candidate.id) && !savedIds.includes(candidate.id)" class="candidate-warning">已发现相同原话的历史记录；确认这是另一笔后可再次保存。</text>
        <button v-if="!savedIds.includes(candidate.id)" class="panel-button candidate-save" form-type="submit" :disabled="busy">{{ duplicateIds.includes(candidate.id) ? '仍要保存这笔' : '确认并保存到月账' }}</button>
...
import { EXPENSE_CATEGORIES, EXPENSE_DIRECTIONS, parseExpenseCents, isExpenseDate, expenseCandidateAlreadySaved, type ExpenseDirection } from '@/utils/expense-ledger'
...
const duplicateIds = ref<string[]>([]), duplicateConfirmedIds = ref<string[]>([])
const preparedText = ref('')
...
watch(() => props.owner, () => { text.value = ''; preparedText.value = ''; consent.value = false; busy.value = false; message.value = ''; candidates.value = []; savedIds.value = []; duplicateIds.value = []; duplicateConfirmedIds.value = []; amounts.value = {}; directions.value = {}; categories.value = {}; dates.value = {}; currentDay.value = localDate() })
watch(text, value => {
  if (!preparedText.value || value.trim() === preparedText.value) return
  candidates.value = []; savedIds.value = []; duplicateIds.value = []; duplicateConfirmedIds.value = []
  message.value = '原话已变化，请重新整理后再保存候选。'
})
function setup(items: ExpenseCandidate[], input: string) {
  preparedText.value = input.trim(); candidates.value = items; savedIds.value = []; duplicateConfirmedIds.value = []; duplicateIds.value = items.filter(item => expenseCandidateAlreadySaved(readToolRecords(), input, item.id)).map(item => item.id); amounts.value = {}; directions.value = {}; categories.value = {}; dates.value = {}
...
async function submit(event: Event) {
  ...
  try {
    ...
    if (owner !== props.owner || reportOwner() !== owner || text.value.trim() !== input) return
    setup(response.candidates, input); text.value = input
...
function saveCandidate(candidate: ExpenseCandidate, event: Event) {
  ...
  const duplicate = expenseCandidateAlreadySaved(readToolRecords(), text.value, candidate.id)
  if (duplicate && !duplicateConfirmedIds.value.includes(candidate.id)) {
    duplicateIds.value = [...new Set([...duplicateIds.value, candidate.id])]
    duplicateConfirmedIds.value = [...duplicateConfirmedIds.value, candidate.id]
    message.value = '找到相同原话的历史记录；如确认是另一笔，请再次点击保存。'
    return
  }
  busy.value = true; message.value = ''
  try {
    const sourceCandidateId = `${text.value.trim().replace(/\s+/gu, ' ')}\u0000${candidate.id}`
    const record = saveToolRecord('expenses', 'entry', { amount: cents / 100, direction, category, date, note: null, sourceCandidateId, sourceCandidateText: text.value, sourceCandidateFragment: candidate.fragment })
