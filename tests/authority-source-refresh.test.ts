...
describe('authority source refresh planning', () => {
  ...
  it('selects only missing and low English enabled sources by default', () => {
    const selected = selectAuthoritySourcesForRefresh({
      sourceCoverage: {
        watchedSources: [
          ...
          { sourceId: 'nhs', count: 0, minimumPublishedRecords: 10, status: 'missing' },
          { sourceId: 'aap', count: 2, minimumPublishedRecords: 10, status: 'low' },
  ...
  it('applies language filtering before explicit source filters and refresh limits', () => {
    const selected = selectAuthoritySourcesForRefresh({
      sourceCoverage: {
        watchedSources: [
          { sourceId: 'chinacdc-nutrition', count: 2, minimumPublishedRecords: 10, status: 'low' },
          { sourceId: 'nhs', count: 0, minimumPublishedRecords: 10, status: 'missing' },
          { sourceId: 'aap', count: 3, minimumPublishedRecords: 10, status: 'low' },
        ],
      },
    }, {
      sourceIds: ['chinacdc-nutrition', 'nhs', 'aap'],
      limit: 1,
    });
