...
export function buildKnowledgeAiAssist(article: Article | null | undefined): KnowledgeAiAssist {
  ...
  return {
    ...
    focusLabel,
    points: [
      summarySentences[0] ? truncateText(summarySentences[0], 66) : `先确认这篇内容是否适合${audienceLabel}。`,
...
export function analyzeDiaryEntry(content?: string | null): DiaryAiAnalysis {
  ...
  return {
    ...
    highlights: (sentences.length ? sentences : [text]).slice(0, 2).map(item => truncateText(item, 58)),
