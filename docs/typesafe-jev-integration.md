...
本文记录贝护后端对 TypeSafe Jev 的最小接入方式。当前 Jev 仅用于工具记录回顾中的固定选项归类；小程序和网页都不能直接调用 TypeSafe，也不能接触 API key。
...
当前适配器只挂在受限的工具记录回顾后端，微信端通过 `VITE_TOOL_AI_ENABLED` 默认关闭；回顾链路只发送用户主动选择的日期、数字和文字，不发送产检原图或儿童照片。正式开放前，必须完成主体/服务资质、隐私政策、供应商数据处理条件、保存期限、删除流程和小程序审核核验。
