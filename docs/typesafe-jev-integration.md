...
本文记录贝护后端对 TypeSafe Jev 的最小接入方式。当前只提供一个后端适配器，供后续工具把自然语言整理成“待确认草稿”；小程序和网页都不能直接调用 TypeSafe，也不能接触 API key。
...
当前适配器还没有挂到用户页面，也没有发送真实产检报告、儿童照片或其它健康资料。接入真实用户数据前，必须完成主体/服务资质、隐私政策、供应商数据处理条件、保存期限、删除流程和小程序审核核验。
