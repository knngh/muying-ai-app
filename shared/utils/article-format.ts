...
function addBlockSpacingToHtml(html: string): string {
  const blockStyles: Array<{ tag: string; style: string }> = [
    ...
    { tag: 'h1', style: 'margin:1.35em 0 0.65em;line-height:1.5;font-size:1.3em;font-weight:700;color:#25313c;text-align:left;' },
    { tag: 'h2', style: HEADING_INLINE_STYLE },
    { tag: 'h3', style: 'margin:1.15em 0 0.55em;line-height:1.6;font-size:1.08em;font-weight:700;color:#334854;text-align:left;' },
...
function isLikelyEnglishHeading(line: string): boolean {
  ...
  if (!trimmed || trimmed.length > 72 || !ENGLISH_HEADING_SIGNAL_PATTERN.test(trimmed)) {
    return false;
  }
...
export function textToRichParagraphHtml(text: string): string {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (isLikelyHeading(paragraph)) {
      html.push(`<h2 style="${HEADING_INLINE_STYLE}">${escapeHtml(paragraph)}</h2>`);
    } else {
      html.push(`<p style="${PARAGRAPH_INLINE_STYLE}">${escapeHtml(paragraph)}</p>`);
    }
  }
